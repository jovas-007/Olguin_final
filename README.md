...
1. Define las variables de entorno o ajusta el diccionario `DB_CONFIG` en `dss/config.py`:
   - `DB_HOST` (por defecto `127.0.0.1` para XAMPP)
   - `DB_PORT` (por defecto `3307`, coincide con tu phpMyAdmin en XAMPP)
   - `DB_USER` (por defecto `root`)
   - `DB_PASSWORD` (por defecto vacío para instalaciones locales de XAMPP)
