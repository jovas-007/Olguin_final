...
def build_olap_views(df_proyectos: pd.DataFrame, df_asignaciones: pd.DataFrame, filtros: Dict):
    ...
    if not proyectos.empty and {"AnioFin", "MesFin"}.issubset(proyectos.columns):
        proyectos = proyectos.copy()
        fecha_fin_cols = (
            proyectos[["AnioFin", "MesFin"]]
            .rename(columns={"AnioFin": "year", "MesFin": "month"})
            .assign(day=1)
        )
        proyectos["FechaFin"] = pd.to_datetime(fecha_fin_cols, errors="coerce")
        proyectos_a_tiempo = (
